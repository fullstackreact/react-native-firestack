...
RCT_EXPORT_METHOD(requestPermissions:(RCTResponseSenderBlock) callback)
{
...
- (void) handleRemoteNotificationReceived:(NSNotification *) n
{
    ...
    [self sendJSEvent:MESSAGING_MESSAGE_RECEIVED_REMOTE props: n];
}
...
RCT_EXPORT_METHOD(getInitialNotification:(RCTResponseSenderBlock)callback){
    ...
    if(localUserInfo){
        callback(@[[NSNull null], @{
                       @"status": @"success",
                       @"launchedFrom": localUserInfo
                       }]);
    }
}

//WORK ON THESE SOME MORE THIS IS UGLY

RCT_EXPORT_METHOD(sendLocal:(NSDictionary *) notification
...
RCT_EXPORT_METHOD(scheduleLocal:(NSDictionary *) notification
                  callback:(RCTResponseSenderBlock) callback)
{
    UILocalNotification* localNotification = [RCTConvert UILocalNotification:notification];
    NSLog(@"scheduleLocal called with notification: %@", notification);
...
//for upstream it makes sense, we could probably use this as a general function for all local,upstream etc. Lets revisit
RCT_EXPORT_METHOD(send:(NSString *) messageId
                  msg: (NSDictionary *) msg
                  ttl: (int) ttl
                  callback:(RCTResponseSenderBlock)callback)
{
    
    int64_t ttl64 = (int64_t) ttl;
    //This is hacky
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString* senderId = [[[FIRApp defaultApp] options] GCMSenderID];
    
    senderId = [NSString stringWithFormat:@"%@@gcm.googleapis.com", senderId];
    [[FIRMessaging messaging] sendMessage:msg
                                       to:senderId
...
RCT_EXPORT_METHOD(sendMessage:(NSDictionary *) details
                  type: (NSString *) type
                  callback:(RCTResponseSenderBlock)callback)
{
...
//send upstream always returns sendDataMessageFailure - pretty sure have to setup the server side component of this to actually see it work
- (void)sendDataMessageFailure:(NSNotification *)notification {
    NSString *messageID = (NSString *)notification.userInfo[@"messageId"];
    NSDictionary *userInfo = notification.userInfo; // contains error info etc
